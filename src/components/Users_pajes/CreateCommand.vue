<template>
  <form @submit.prevent="regCommand">
    <upper/>
    <div id="loging" >
      <div id="login-form">
        <fieldset>
          <label class="loggo" id="main_title">СОЗДАНИЕ КОМАНДЫ</label>
          <div
              class="err_message"
              v-if="this.flagNoCommand">
            Команда уже существует
          </div>

          <input type="text" placeholder="ИМЯ КОМАНДЫ" title="имя команды" v-model.trim="nameCommand"
                 v-bind:class="{'error_in_data': ($v.nameCommand.$dirty && !$v.nameCommand.required),
                 'in_data': true}">
...
export default {
  ...
  methods: {
    ...
    regCommand() {
      ...
      let formDataReg = {
        name: this.nameCommand,
        login: Vue.$cookies.get('login')
      }

      axios({
        method: 'post',
        url: 'https://ict-tagall.herokuapp.com/main/app/team/add',
        data: formDataReg
      }).then(resp => {
        this.$router.push('/lk')
      }).catch(err => {
        this.flagNoCommand = true
        return
      })
    }
  },
